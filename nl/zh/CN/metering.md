...
{: caption="表 6. 每日最大使用量及每月平均值计算" caption-side="top"}
...
您可以使用档位配置来以不同方式汇集单位数量：在提交使用量时发送的数量、在“使用情况仪表板”中显示的使用量，以及用于确定费率和成本计算的数量。以下示例演示了这些方案：
...
|按比例分配|将每个资源的每日单价 (P) 乘以每日使用数量 (Q) 等于每日总金额。总费用涉及累计该月份内所有天数的费用。|T= (pd * Q1) + ...+(Pd *Qn)| <ul><li>P= $30</li><li>Pd（每日价格）=$30/30=$1（假定一个月有 30 天）</li><li>T1= $1 * 1 =$1</li><li>T2 = $1 * 0 =$0</li><li>Tn = 1 * 1 =$1</li><li>T = $1 + $0 +...+$1 = $5000</li></ul>     |
...
{: caption="表 7. 定价模型" caption-side="top"}
