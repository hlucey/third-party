...
*	产品提供者负责自动执行使用量提交，包括重试失败响应的自动提交。{{site.data.keyword.Bluemix_notm}} 没有为失败的提交提供重试功能。有关更多信息，请参阅[提交使用情况记录](/docs/third-party/submitusage.html#submitting-usage-records)中的状态码和操作表。
...
|第 15 天到第 30 天|0|0 / 1| (5.5 + 3.5 + (1 \* 12) + (0  \* 15) / 30 | 0.7333（第 30 天结束时）|
...
您可以使用档位配置来以不同方式汇集单位数量：提交使用量中发送的使用量、在“使用情况仪表板”中显示的使用量，以及用于确定费率和成本计算的内容。以下示例演示了这些方案：
...
|按比例分配|将每个资源的每日单价 (P) 乘以每日使用数量 (Q) 等于每日总金额。总费用涉及累计一个月内所有天数的费用。|T= (pd * Q1) + ...+(Pd *Qn)| <ul><li>P= $30</li><li>Pd（每日价格）=$30/30=$1（假定一个月有 30 天）</li><li>T1= $1 * 1 =$1</li><li>T2 = $1 * 0 =$0</li><li>Tn = 1 * 1 =$1</li><li>T = $1 + $0 +...+$1 = $5000</li></ul>     |
