...
您需要每小时提交一次所有活动服务实例的使用量。不报告使用量会导致 IBM 的收入损失，进而导致产品提供者的收入份额损失。
{:shortdesc}
...
2. 使用量记录将根据计量模型进行聚集，并且总数量将显示在 {{site.data.keyword.Bluemix_notm}} 控制台中的“使用情况仪表板”上。
3. 将使用计量过程中的聚集单位数量，应用成本，并计算用户对服务实例的欠费金额。
4. 在月底，最终计算的成本即为向用户生成的金额。
...
* 使用量记录是事实。创建使用量记录后，不要对其进行更新。成功创建使用量记录时会指定位置。如果返回错误代码，请查看可能必须执行的[操作](#actions)。
...
* 与计量服务的交互不应与其他任何服务组合使用。即使计量以供应和取消供应实例来开始和结束，也应单独处理请求。
...
* 建议服务提供者每小时提交一次使用量，这样最终用户就不会看到从资源使用到成本反映在其帐户中之间出现极大延迟。
...
要使用 {{site.data.keyword.Bluemix_notm}} 计量服务来报告使用量数据，请实现计量服务 API 以报告产品的使用量数据。有关更多详细信息，请参阅[公共 API 文档](https://ibm-bluemix-docs.github.io/usage-submission/)。  
...
|404|计量定义尚未上线。请与 BSS 计量团队合作，检查是否资源已上线，然后重新提交使用量记录。|
