...
您可以保存在资源管理控制台中的进度，以后返回来添加更多信息。资源管理控制台旨在帮助您管理服务的生命周期。如果当时没有填完所有字段，也没关系。
{: tip}
...
1. 在资源管理控制台中，单击**产品**页面，然后单击**列表页面**选项卡。**列表页面**定义在 {{site.data.keyword.Bluemix_notm}} 产品的服务仪表板中显示的元数据。填写所有必需值，然后单击**保存**。资源管理控制台会向 {{site.data.keyword.Bluemix_notm}} Identity and Access Management (IAM) 初始注册您的服务。这将显示服务已向 IAM 注册的通知。稍后您将使用 IAM 执行更多操作。
...
**注**：稍后您需要返回到 IAM 页面来提供`重定向 URI`。在执行其他一些开发以构建认证流程之前，您不会拥有此值。后面的步骤将帮助指导您识别重定向 URI 值。
