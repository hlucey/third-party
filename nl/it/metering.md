...
*	Il provider di offerte è responsabile dell'automazione dell'inoltro dell'utilizzo, compresa l'automazione che riprova le risposte di errore. {{site.data.keyword.Bluemix_notm}} non fornisce una funzione di nuovo tentativo per gli inoltri non riusciti. Vedi i codici di stato e la tabella delle azioni in [Inoltro dei record di utilizzo](/docs/third-party/submitusage.html#submitting-usage-records) per ulteriori informazioni.
...
*	{{site.data.keyword.Bluemix_notm}} è configurato per un ciclo di fatturazione mensile e l'ora è rappresentata in UTC.
...
Vedi [Come calcolare i tuoi costi](https://console.bluemix.net/docs/billing-usage/estimating_costs.html#cost) per informazioni generali sui prezzi.
...
#### Media quota proporzionale giornaliera
...
#### Massimo quota proporzionale giornaliero
...
Puoi utilizzare la configurazione di ridimensionamento per compilare la quantità unitaria differentemente da quello che viene inviato negli inoltri dell'utilizzo a quello che viene visualizzato nel dashboard di utilizzo e quello che viene infine utilizzato per i calcoli di valutazione e costo. I seguenti esempi illustrano degli scenari di quando questi valori devono essere configurati:
...
| Proration      | Moltiplica il prezzo unitario giornaliero per risorsa (P) per la quantità di utilizzo giornaliero (Q) per ottenere l'importo giornaliero totale. L'addebito totale comporta l'accumulo degli addebiti per tutti i giorni nel mese indicato.         | T= (pd * Q1) + ...+(Pd *Qn)     | <ul><li>P= $30</li><li>Pd (prezzo giornaliero) =$30/30=$1 (presumendo 30 giorni in un mese)</li><li>T1= $1 * 1 =$1</li><li>T2 = $1 * 0 =$0</li><li>Tn = 1 * 1 =$1</li><li>T = $1 + $0 +...+$1 = $5000</li></ul>     |
