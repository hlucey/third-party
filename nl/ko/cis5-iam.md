...
**참고:** 이 토큰은 1시간 동안 유효하며, 1시간 동안에는 필요한 만큼 여러 번 다시 사용될 수 있습니다. `dashboard_url`에 대한 모든 액세스에 이 요청을 수행하지 않도록 이 토큰을 캐싱할 것을 권장합니다.
...
클라이언트 ID로 작성된 사용자 액세스 토큰은 서비스 API 액세스에만 사용될 수 있습니다. 이 토큰을 사용하는 다른 클라우드 API에 대해 요청하면 사용자가 적절한 정책을 구성한 경우에도 액세스가 거부됩니다.
