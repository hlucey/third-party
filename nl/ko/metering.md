...
*	써드파티 오퍼링 제공자는 무료 사용제의 경우 사용량을 제출할 필요가 없습니다.
*	써드파티 오퍼링 제공자는 월별 구독 사용제의 경우 사용량을 제출할 필요가 없습니다.
*	측정량 기반 플랜의 경우 모든 오퍼링 제공자는 시간별 사용량을 제출해야 합니다(라이트 플랜은 15분에서 1시간마다 제출해야 함).
*	오퍼링 제공자는 실패 응답을 다시 시도하는 자동화를 포함하여 사용량 제출을 자동화해야 합니다. {{site.data.keyword.Bluemix_notm}}는 실패한 제출에 대한 재시도 기능을 제공하지 않습니다. 자세한 정보는 [사용량 레코드 제출](/docs/third-party/submitusage.html#submitting-usage-records)에서 상태 코드 및 조치 테이블을 참조하십시오.
*	현재 달의 사용량 레코드는 늦어도 다음 달 2일까지 제출해야 합니다.
*	{{site.data.keyword.Bluemix_notm}}는 월별 청구 주기에 맞게 구성되며 시간은 UTC로 표시됩니다.
*  오퍼링 제공자는 사용량 제출을 테스트하고 월별 청구 주기를 계산하는 방법을 알려주는 결과를 검증해야 합니다.

가격 책정에 대한 일반 정보는 [비용 계산 방법](https://console.bluemix.net/docs/billing-usage/estimating_costs.html#cost)을 참조하십시오.
...
<dt>단위 이름</dt>
<dd>기본 이름이 오퍼링과 관련이 없는 경우 단위의 설명 이름입니다.</dd>
</dl>
...
| 15일 - 30일        | 0                | 0 / 1         | (5.5 + 3.5 + (1 * 12) + (0  * 15) / 30 | 0.7333(20일 EOD)                                |
...
스케일링 구성을 사용하여 단위 수량을 사용량 제출에서 보낸 양과 다르게 사용량 대시보드에 표시되는 양으로 컴파일하고 마지막으로 등급 지정 및 비용 게산에 사용되는 양으로 컴파일할 수 있습니다. 다음 예제는 이러한 값을 구성해야 하는 시나리오에 대해 설명합니다.
...
### 가격 책정 구성에 있는 것보다 상세한 정보를 원하는 경우
메트릭 가격을 $X/기가바이트로 책정하지만 이를 메가바이트로 보내려고 합니다. 메트릭 가격이 $1/기가바이트이지만 사용자가 0.5메가바이트를 사용한 경우 가격 책정이 기가바이트 단위로 구성되어 있으므로 $1가 부과됩니다. `scale` 1024를 **등급 지정** 구성에 추가하고 `clip`을 `true`로 설정합니다.
...
다음 표에서는 사용 가능한 가격 책정 모델에 대한 자세한 정보를 제공합니다. 사용 가능한 여러 메트릭에 대해 연관된 가격 책정 모델을 선택합니다.
